...
void AlarmApp::initNewAlarm(int index) {
    if (index >= 0 && index < MAX_ALARMS) {
        alarms[index].isUsed   = true;
        alarms[index].isOpen   = true;
        alarms[index].hour     = 7;
        alarms[index].minute   = 0;
        alarms[index].weekMask = DEFAULT_WEEK_MASK;
    }
}
...
void AlarmApp::sortAlarms() {
    for (int i = 0; i < MAX_ALARMS - 1; i++) {
        for (int j = 0; j < MAX_ALARMS - 1 - i; j++) {
            bool swap = false;
            if (!alarms[j].isUsed && alarms[j + 1].isUsed) {
                swap = true;
            } else if (alarms[j].isUsed && alarms[j + 1].isUsed) {
                int timeA = alarms[j].hour * 60 + alarms[j].minute;
                int timeB = alarms[j + 1].hour * 60 + alarms[j + 1].minute;
                if (timeA > timeB) swap = true;
            }
            if (swap) {
                AlarmSlot temp = alarms[j];
                alarms[j]      = alarms[j + 1];
                alarms[j + 1]  = temp;
            }
        }
...
void AlarmApp::saveAlarmData() {
    if (sys) {
        sys->storage.saveStruct("alarm_data", alarms);
    }
}

void AlarmApp::loadAlarmData() {
    if (sys) {
        sys->storage.loadStruct("alarm_data", alarms);
    }
}
    ...
    if (viewState == VIEW_LIST) {
        ...
        if (isEditingValue) {
            if (editFocus == FOCUS_TIME_HOUR) {
                alarms[listIndex].hour++;
                if (alarms[listIndex].hour >= 24) alarms[listIndex].hour = 0;
            } else if (editFocus == FOCUS_TIME_MINUTE) {
                alarms[listIndex].minute++;
                if (alarms[listIndex].minute >= 60) alarms[listIndex].minute = 0;
            }
...
void AlarmApp::onkeyDown() {
    ...
    if (viewState == VIEW_LIST) {
        ...
        if (isEditingValue) {
            if (editFocus == FOCUS_TIME_HOUR) {
                if (alarms[listIndex].hour == 0) alarms[listIndex].hour = 23;
                else alarms[listIndex].hour--;
            } else if (editFocus == FOCUS_TIME_MINUTE) {
                if (alarms[listIndex].minute == 0) alarms[listIndex].minute = 59;
                else alarms[listIndex].minute--;
            }
        } else if (editFocus == FOCUS_WEEK_ROW && isEditingWeekRow) {
            weekDayCursor--;
...
void AlarmApp::onkeySelect() {
    if (viewState == VIEW_LIST) {
        ...
        if (editFocus == FOCUS_BACK_BTN) {
            ...
            alarms[listIndex].isOpen = !alarms[listIndex].isOpen;
            ...
            if (isEditingWeekRow) {
                alarms[listIndex].weekMask ^= (1 << weekDayCursor);
            } else {
...
void AlarmApp::handleLongPressLoop(float dt) {
    ...
    if (viewState == VIEW_LIST && listIndex >= 0 && listIndex < count) {
        ...
        if (deleteProgress >= 1.0f) {
            alarms[listIndex].isOpen = false;
            alarms[listIndex].isUsed = false;
            sortAlarms();
...
void AlarmApp::drawListView(int offsetX) {
    ...
    for (int i = 0; i < totalItems; i++) {
        ...
        if (i < count) {
            char buf[16];
            sprintf(buf, "%02d:%02d", alarms[i].hour, alarms[i].minute);
    ...
    if (listIndex >= -1 && listIndex < totalItems) {
        ...
        if (listIndex == count) {
            ...
            if (deleteProgress > 0.1f) {
                ...
                bool isOpen = alarms[listIndex].isOpen;
...
void AlarmApp::drawEditView(int offsetX) {
    ...
    int boxX = offsetX + 88;
    const char* swStr = alarms[listIndex].isOpen ? STR_ALARM_ON[L] : STR_ALARM_OFF[L];
    ...
    sprintf(hourBuf, "%02d", alarms[listIndex].hour);
    sprintf(minBuf, "%02d", alarms[listIndex].minute);
    ...
    for (int i = 0; i < 7; i++) {
        ...
        bool isActive = (alarms[listIndex].weekMask >> i) & 1;
